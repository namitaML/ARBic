...
{
    const continuous *da = (const continuous *)a;
    const continuous *db = (const continuous *)b;
...
void partitionMapping()
{
	...
	for(i=0;i<rows;i++)
	{
		...
		for(j=0;j<cols;j++)
		{
			aij = (arr[i][j] - min + 0.0001)/temp;
			arr[i][j] = ceil(aij * po->DIVIDED);		
		}
		min=10000.0;
		max =-10000.0;
		temp = 0.0;
...
void discretizeSq()
{
	...
	for (row = 0; row < rows; row++)
	{
		...
		for (col = 0; col < cols; col++)
		{
		/*	charset_add(symbols, dis_value(arr[row][col],po->DIVIDED, small, cntl, big, cntu));*/
...
void discretize_Density()
{
    int row, col,col2,count,max_count,center;
    //po->density;
    //po->DIVIDED;
    continuous rowdata[cols];
    int i;
    ...
    for (row=0;row<rows;row++)
    {
        ...
        for(col=0;col<cols;col++)
        {
            ...
            {
                if(abs(rowdata[col2]-rowdata[col])<po->density)
                    {
                        count=count+1;                                          
                    }
            }
        ...
        for (col=0;col<cols;col++)
        {
            arr_c[row][col]=(int((rowdata[col]-rowdata[center])/po->density)+1)/2;
        }    
    }
return;
}
