...
/* ben lai zhe li shi zhu xiao de 
#define AllocVar(pt,type) (pt = (type)xmalloc(sizeof(*pt)))
#define AllocArray(pt, type, size) (pt = (type)xmalloc(sizeof(*pt) * (size)))
...
extern discrete *lcs_length;/*clc*/
extern char **lcs_tags;/*clc*/
extern discrete *lcs_rowNo;/*clc*/
...
typedef struct Prog_options{
	...
	bool IS_const;// i add
	int MODE;// i add
	int COL_MAXWIDTH;	//i add
	int COL_WIDTH;
	int DIVIDED;
	int DIVIDED2;
	int SCH_BLOCK;
	int RPT_BLOCK;
	double ROW_MINWIDTH;//i add
	double FILTER;
	double QUANTILE;
	double QUANTILE2;//lxy g
	double density;
	double TOLERANCE;
	int Shuffle;
